	...
	{#if (comparison && colorMap)}
		...
		>
			...
			<ColoredBarChart>
				...
				{#each comparison.sectorWeightings as fws}
					...
					>
						<ColoredBarEntry color="{fws.fundId ===  $page.url.searchParams.get('fundOne') ? '#f582ae' : '#008080'}"
														 width={100} />
					</ColoredBar>
				{/each}
			</ColoredBarChart>
		...
		<TertiaryContainer on:containerClicked={() => fundOneNonOverlappingHoldingsElem.growDiv()}>
			<PulldownElement
				...
				selector="f1-non-overlapping-funds"
				size="text-xs"
			>
				<div class="pt-3">
					<List>
		...
		<TertiaryContainer on:containerClicked={() => fundTwoNonOverlappingHoldingsElem.growDiv()}>
			<PulldownElement
				...
				selector="f2-non-overlapping-funds"
				size="text-xs"
			>
				<div class="pt-3">
					<List>
