...
import type { FilterPortfolioFundHoldingsResponse__Output } from '$lib/proto/proto/FilterPortfolioFundHoldingsResponse.ts';
...
export const load = (async ({ request, params, url, route }) => {
	...
	Object.keys(detailsResp.data?.portfolioFundSectorWeightings).forEach((key) => {
		const value: PortfolioFundSectorWeighting__Output =
			detailsResp.data?.portfolioFundSectorWeightings[key];
	...
	sectorWeightings = sectorWeightings.sort(
		(x, y) => y.weighting.totalPercentage - x.weighting.totalPercentage
	);
	const holdings: Holding[] = detailsResp.data.portfolioFundHoldings.map((x) => ({
		...
		funds: x.funds
			.map((f) => ({ fundId: f.fundId, ratiodPercentage: f.ratiodPercentage }))
