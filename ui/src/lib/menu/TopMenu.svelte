<script lang="ts">
    ...
    export let profilePictureURL: string | null = ""
...
<div class="relative">
    ...
    >
        {#if (profilePictureURL)}
            <img class="h-8 w-8 rounded-full" src="{profilePictureURL}" alt="" referrerpolicy="no-referrer">
        {:else}
            <svg class="h-8 w-8 fill-tertiary" version="1.1" id="Layer_1" x="0px" y="0px"
    ...
    {#if (isMenuOpen)}
        ...
        >
            <li class="flex">
                <a
                        class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-xl hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200"
                        href="/api/auth/login"
                >
                    <svg
                            class="w-4 h-4 mr-3 fill-tertiary"
                            aria-hidden="true"
