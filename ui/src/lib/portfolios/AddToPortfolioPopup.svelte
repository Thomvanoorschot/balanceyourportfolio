<script lang="ts">
	...
	const setValues = (portfolio: Portfolio__Output | undefined) => {
		...
		if (portfolio.entries.some((x: PortfolioListItem__Output) => x.fundId === $page.params.id)) {
			existingPortfolioFund = portfolio.entries.find(
				(x: PortfolioListItem__Output) => x.fundId === $page.params.id
			);
		} else {
...
{#if portfolios}
	<div class="flex flex-col p-8 w-full bg-primary gap-4 rounded-xl">
		...
		{#if existingPortfolioFund}
			<Input placeholder="Amount" bind:value={existingPortfolioFund.amount} type="number" />
			<TertiaryButton
		...
		{:else}
			<Input placeholder="Amount" bind:value={newPortfolioFundAmount} type="number" />
			<TertiaryButton
